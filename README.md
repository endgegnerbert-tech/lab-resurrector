...
Render's free tier puts the server to sleep after 15 minutes of inactivity. The first visit after sleep takes 30-60 seconds to wake up (cold start). Render health checks (`render.yaml` `healthCheckPath`) keep the service alive as long as traffic flows.
